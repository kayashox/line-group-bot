...
export async function generateReply(userMessage, systemPrompt, knowledge = [], recentMessages = []) {
  ...
  try {
    ...
    console.error("[ai] primary generation failed:", err?.message || err);

    // フォールバック: コンテキストを極小化して再試行
    const fallbackSystemPrompt = truncateText(systemPrompt, 2400);
    const fallbackUserMessage = `次の相談に、短く具体的に答えてください。\n\n${truncateText(userMessage, 500)}`;
